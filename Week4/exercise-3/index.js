...
class ShoppingCart {
  ...
  addItem(productId, quantity = 1) {
    try {
      ...
      product.stock -= quantity; // quantity miktarı her product için düşüyor.
  ...
  calculateTotal() {
    this.total = this.items.reduce((sum, item) => {
      ...
      return sum + item.price * item.quantity; // this.items ile ürün kontrol edildi
    }, 0);
  ...
  showError(message) {
    ...
    if (errorElement) {
      errorElement.textContent = message; // mesajlar artık yanyana yazılmıyor
    }
  }
